 ...
 * 2. `[]` system_program: {@link PublicKey} Auto-generated, for account initialization
 */
export const reset = (args: ResetArgs): TransactionInstruction => {
    const data = serialize(
        {
            struct: {
                id: "u8",
 ...
 * 2. `[]` system_program: {@link PublicKey} Auto-generated, for account initialization
 */
export const resetSendAndConfirm = async (
    args: Omit<ResetArgs, "feePayer" |"greetingAccount"> & { 
        signers: { feePayer: Keypair,  greetingAccount: Keypair, }
 }
): Promise<TransactionSignature> => {
    const trx = new Transaction();
